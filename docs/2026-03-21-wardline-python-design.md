...
- `AuthoritativeField` descriptor: stores in `obj.__dict__["_wd_auth_{name}"]`, raises `AuthoritativeAccessError` on access-before-set. **Storage key namespacing:** The storage key uses a `_wd_auth_` prefix rather than `_authoritative_` to avoid collisions — a class with both a descriptor named `status` and a raw attribute named `_authoritative_status` would silently collide under the longer prefix. Add a test for this collision scenario to `test_descriptors.py`. Known residual: `__dict__` manipulation bypasses the sentinel — test this bypass explicitly.
...
**`except*` (ExceptionGroup) scope:** Python 3.11 introduced `except*` blocks (`ast.TryStar` node) for `ExceptionGroup` handling. The MVP scanner targets Python 3.12+ codebases, which may use `except*`. PY-WL-004 and PY-WL-005 MUST handle `ast.TryStar` nodes in addition to `ast.ExceptHandler`. An `except*` block that catches broadly or silently is the same anti-pattern as a broad `except` — the AST node type differs but the semantic concern is identical. **Runtime compatibility guard:** `ast.TryStar` is not guaranteed to be present in `ast.__all__` across all Python 3.12 builds. The implementation MUST use a runtime check: `_HAS_TRY_STAR = hasattr(ast, 'TryStar')` and guard the `ast.TryStar` visitor path with this flag. If `_HAS_TRY_STAR` is `False`, the scanner SHOULD emit a WARNING at startup: "`except*` blocks will not be analysed — `ast.TryStar` not available in this Python build." Add corpus specimens for both `ast.ExceptHandler` and `ast.TryStar` patterns. The `ast.TryStar` specimens must include a `skip_condition` field in the specimen YAML: `skip_condition: "not hasattr(ast, 'TryStar')"` so that corpus verification does not fail on builds where the node type is absent.
